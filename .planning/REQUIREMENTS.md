...
- [x] **LOG-01**: Приложение пишет логи в файл ~/.yurteg/logs/yurteg.log с ротацией (5MB × 3)
...
- [x] **DUX-01**: Приложение имеет иконку «Ю» в индиго — .icns (macOS) и .ico (Windows) во всех размерах (16/32/128/256/512px)
- [x] **DUX-02**: При запуске PyInstaller-бандла показывается splash/loading вместо пустоты (3-5 сек распаковка)
- [x] **DUX-03**: При первом запуске без интернета — понятное сообщение «подключитесь к интернету для первой настройки» вместо краша
- [x] **DUX-04**: Перед скачиванием моделей проверяется свободное место на диске (>1.5GB), при нехватке — предупреждение
