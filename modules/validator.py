"""Модуль валидации метаданных договоров.
...
def _validate_l1(metadata: ContractMetadata) -> list[str]:
    ...
    if not metadata.contract_type:
        warnings.append("L1: отсутствует тип договора (contract_type)")
    if not metadata.counterparty:
        warnings.append("L1: отсутствует контрагент (counterparty)")
    if not metadata.subject:
        warnings.append("L1: отсутствует предмет договора (subject)")
...
def _validate_l2(metadata: ContractMetadata, config: Config) -> list[str]:
    ...
    # Тип договора — нечёткое сравнение
    if metadata.contract_type:
        _, score = _fuzzy_match(metadata.contract_type, config.contract_types)
        if score < 0.8:
            warnings.append(
                f"L2: нестандартный тип договора: «{metadata.contract_type}»"
            )

    # Сумма
    if metadata.amount:
        digits = re.findall(r'\d+', metadata.amount.replace(" ", ""))
        if not digits:
    ...
    if metadata.subject:
        if len(metadata.subject) < 5:
            warnings.append(
                f"L2: слишком короткий предмет договора ({len(metadata.subject)} символов)"
            )
